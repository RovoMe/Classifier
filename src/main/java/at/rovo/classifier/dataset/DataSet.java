...
public abstract class DataSet
{
	...
	public void addColumnSetView()
	{
		...
		for (int i = 0; i < attributeCount; i++)
		{
			...
			{
				attributes[i] = new ContinuousAttribute(name,
						transposedTrainData[i]);
			}
			else
			{
				String[] nominalValues = metaData
						.getAttributeNominalValuesAt(i);
				attributes[i] = new DiscreteAttribute(name, nominalValues,
						transposedTrainData[i]);
			}
		}
	}
	 ...
	 * 
	 * @param The number of different class values
	 */
