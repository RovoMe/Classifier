...
/**
 * <p>A data set loaded from UCI dataset-format files.</p>
 * <p>This implementation is based on the implemented by Xu and He in their C45 
 * framework. They assumed that the class attribute is at the same position in 
 * the meta-data file (.names) as in the data file (.data). However this 
 * assumption is not true in most UCI data sets.</p>
 * 
 * @author Xiaohua Xu
 * @author Ping He
 * @author Roman Vottner
 * @see http://www.cs.washington.edu/dm/vfml/appendixes/c45.htm
 */
...
{	
	 ...
	 * 
	 * @param baseName The file name to load the data from
	 */
	...
	public MetaData loadMetaData(String filename)
	{
		...
		List<Boolean> tempIsContinuous = new ArrayList<Boolean>();
		// The 'tempNominalValues' ArrayList dynamically reads the nominal 
		// values of the discrete attributes in
		List<String[]> tempNominalValues = new ArrayList<String[]>();
		...
		{			
			...
			while ((line = bf.readLine()) != null)
			{
				...
				{	
					...
					this.setClassValues(classes.toArray(new String[0]));
					tempAttributeNames.add("class");
					tempIsContinuous.add(false);
					tempNominalValues.add(classes.toArray(new String[0]));
				...
				else
				{
					...
					else
					{
						...
						tempNominalValues.add(values.toArray(new String[0]));
					}
				}
		...
		return new MetaData(tempAttributeNames.toArray(new String[0]),
				tempIsContinuous.toArray(new Boolean[0]),
				tempNominalValues.toArray(new String[0][]));
	}
	...
	/**
	 * Extracts the attribute values from a single String separated with
	 * specified delimiter
	 * 
	 * @param source The String to be extracted
	 * @param delimiterString The delimiter with which the attribute values are 
	 *                        separated
...
// 		String[] result = source.split(delimiterString);
	...
	/**
	 * Swaps the position of the class labels in the MetaData file as there is
	 * only one swap necessary instead of n if we would swap the classes within
	 * the data extraction.
	 * 
	 * @param metaPos The position of the class label in the meta file
	 * @param dataPos The position of the class label in the data file
	 */
