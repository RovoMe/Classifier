...
public class DrumTrainingData<F, C> implements AppendableData<DrumTrainingData<F, C>>
{
	...
	public byte[] toBytes()
	{
		...
		System.arraycopy(DrumUtil.int2bytes(featureSize), 0, totalBytes, pos, pos += 4);
		...
		System.arraycopy(DrumUtil.int2bytes(i+1), 0, totalBytes, pos, pos += 4);

		for (i=0; i < cats.size(); i++)
		{
			byte[] catNameBytes = catBytes[i];
			// copy the length of the cat name into the byte array
			System.arraycopy(DrumUtil.int2bytes(catNameBytes.length), 0, totalBytes, pos, pos += 4);
			...
			System.arraycopy(DrumUtil.int2bytes(this.catCount.get(cats.get(i))), 0, totalBytes, pos, pos += 4);
		}
		return totalBytes;
	}
	...
	public DrumTrainingData<F, C> readBytes(byte[] data)
	{
		...
		System.arraycopy(data, pos, featureSizeBytes, 0, 4); pos += 4;
		int featureSize = DrumUtil.bytes2int(featureSizeBytes);
		// read the feature
		byte[] featureBytes = new byte[featureSize];
		System.arraycopy(data, pos, featureBytes, 0, featureSize); pos += featureSize;
		F feature = this.convertBytesToObject(featureBytes, featureClass);
		// read the number of categories
		byte[] numCatBytes = new byte[4];
		System.arraycopy(data, pos, numCatBytes, 0, 4); pos += 4;
		int numCat = DrumUtil.bytes2int(numCatBytes);
		...
		for (int i=0; i < numCat; i++)
		{
			...
			System.arraycopy(data, pos, catNameLengthBytes, 0, 4); pos += 4;
			int catNameLength = DrumUtil.bytes2int(catNameLengthBytes);
			// read the category name
			byte[] catNameBytes = new byte[catNameLength];
			System.arraycopy(data, pos, catNameBytes, 0, catNameLength); pos += catNameLength;
			...
			System.arraycopy(data, pos, catCountBytes, 0, 4); pos += 4;
			int catCount = DrumUtil.bytes2int(catCountBytes);
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof DrumTrainingData)
		{
			...
			if (data.feature.equals(this.feature)
					&& data.catCount.equals(this.catCount))
				return true;
		}
		return false;
	}
