...
/**
 * Implementation of a naive Bayes classifier which uses probabilities extracted from training examples to divide
 * certain candidates into their appropriate classes.
 * <p/>
 * The current implementation only supports a multinominal event model.
 * <p/>
 * This implementation was originally based on the code presented in "Programming Collective Intelligence" by Toby
 * Segaran (ISBN: 978-0-596-52932-1; 2007) but has changed massively since the beginning.
 *
 * @param <F>
...
{
    protected static Logger LOG = LoggerFactory.getLogger(NormalNaiveBayes.class.getName());

    // for faster lookup
    /** Contains the probability for each category **/
    private Map<C, Double> catProb = null;
    /** Contains the actual trained data **/
    protected NBTrainingData<F, C> trainingData = null;
    /** Defines the actual strategy to store trained data **/
    ...
    /**
     * Create a new instance of this class and sets its name to the value of the provided argument.
     *
     * @param name
    ...
    /**
     * Calculates the a-priori probability for a certain category.
     * <p/>
     * It therefore counts all entries labeled with this category and divides the sum through the total number of
     * entries.
     *
     * @param category
     *         The category whose probability should be calculated
     *
    ...
    public double getCategoryProbability(C category)
    {
        ...
        {
            LOG.debug("   P('{}') = {}/{} = {}", category, this.trainingData.getNumberOfSamplesForCategory(category),
    ...
    /**
     * Calculates the probability for a certain feature.
     * <p/>
     * This method runs through every defined category and sums up the conditional probability of the feature given the
     * category <em>P(X|C)</em> times the probability of the category <em>P(C)</em> itself.</p>
     *
     * @param feature
    ...
    /**
     * Calculates the probability for a certain set of features.
     *
     * @param features
     *         The feature whose probability should be calculated
     *
