 ...
 * categories or classes these features occur. In contrast to {@link NormalNaiveBayes}, this implementation allows to
 * define a smoothing value, which if set to 0 does not have an effect on the calculated outcome and therefore returns
 * the same result as the other implementation. If the smoothing factor is between 0 and 1 a Lidstone smoothing will be
 * applied whereas if the smoothing factor is 1 a Laplace smoothing will be applied to the calculated probability.
 *
 * @param <F>
