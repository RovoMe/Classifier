...
/**
 * A calculator specifically used to compute Gain, GainRatio and Entropy
 * criteria for different data distribution.
 * 
 * @author Ping He
 * @author Xiaohua Xu
 */
public class GainCalculator
{
	 ...
	 *            the weight distribution of the data in different attribute
	 *            branch values
	 * @param classDistribution
	 *            the weight distribution of the data in different classes and
	 *            different attribute branch values.
	 * @param unknownRatio
	 *            the ratio of the missing data
	 * @return Gain value of the specified data distribution
	 */
	public static float computeGain(float stateEntropy,
			float[] branchDistribution, float[][] classDistribution,
			float unknownRatio)
	{
		int reasonableSubset = 0;
		for (int i = 1; i < branchDistribution.length; i++)
		{
			if (branchDistribution[i] > Parameter.MINWEIGHT
					- Parameter.PRECISION)
				reasonableSubset++;
		}
	 ...
	 */
	public static float computeStateEntropy(float[][] classDistribution,
			float knownWeight)
	{
	 ...
	 */
	public static float computeSplitInfo(float[] branchDistribution,
			float totalWeight)
	{
		return computeTotalInformation(branchDistribution, totalWeight)
				/ totalWeight;
	}
	 ...
	 * 
	 * @param branchDistribution
	 *            the weight distribution of the cases in different branch
	 *            values
	 * @param totalWeight
	 *            the total weight of all the cases
	 * @return the splitInfo evaluation
	 */
	private static float computeTotalInformation(float[] branchDistribution,
			float totalWeight)
	{
	 ...
	 */
	public static float computeGainRatio(float gain, float splitInfo,
			float minGain)
	{
		if (gain >= minGain - Parameter.PRECISION
				&& splitInfo > Parameter.PRECISION)
		{
	 ...
	 *            values
	 * @param classDistribution
	 *            the weight distribution of the cases in different classes
	 *            among each different branch value.
	 */
	private static float computeStandardEntropy(float[] branchDistribution,
			float[][] classDistribution)
	{
		...
		for (int i = 1; i < branchDistribution.length; i++)
		{
			float infoPart = (branchDistribution[i] == 0) ? 0
					: computeTotalInformation(classDistribution[i],
							branchDistribution[i]);
