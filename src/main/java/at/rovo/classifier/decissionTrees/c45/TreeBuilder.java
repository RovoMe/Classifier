...
class TreeBuilder
{
	...
	TreeBuilder(DataSet dataSet, AttributeDelegate[] attributeDelegates)
	{
		...
		this.attributeDelegates = 
				new AttributeDelegate[dataSet.getAttributeCount()];
		...
		for (Attribute attribute : dataSet.getAttributes())
		{
			if (attribute instanceof ContinuousAttribute)
				...
				this.attributeDelegates[attributeIndex] = 
				new DiscreteAttributeDelegate((DiscreteAttribute) attribute);
			attributeIndex++;
		}
	 ...
	 * 
	 * @param first The start(inclusive) index of the train data used for tree
	 *              node construction.
	...
	private TreeNode constructTreeNode(int first, int last)
	{
		...
		// If any of the leaf conditions is satisfied, return the Leaf
		// tree node
		if (content.satisfyLeafNode(Parameter.MINWEIGHT)
				|| this.candidateTestAttrCount <= 0)
		{
		...
		Attribute testAttribute = selectTestAttribute(first, last,
				testAttributeInfo);
		...
		AttributeDelegate testAttributeDelegate = 
				this.attributeDelegates[testAttributeIndex];
		...
		if (!(testAttribute instanceof ContinuousAttribute))
		{
			...
			testBranchDistri = new float[((DiscreteAttribute) testAttribute)
					.getNominalValuesCount() + 1];
		...
		{
			groupBegin = testAttributeDelegate.groupForward(first, last, -1,
					testBranchDistri);
		}
		...
		for (int index = 0; index < testBranchDistri.length - 1; index++)
		{
			...
			int split = testAttribute instanceof ContinuousAttribute ? cutRank
					: index;
			...
			{
				nextGroupBegin = testAttributeDelegate.groupForward(groupBegin,
						last, split, testBranchDistri);
			}
			...
			if (groupBegin == nextGroupBegin)
			{
				...
				aChild = new LeafNode(new TreeNodeContent(0, null, content
						.getClassification(), 0));
			}
			...
			else if (groupBegin > missingBegin)
			{
				...
				float ratio = testBranchDistri[index + 1]
						/ (content.getTrainWeight() - testBranchDistri[0]);
				...
				missingBegin = testAttributeDelegate.groupBackward(
	 ...
	 * 
	 * @param first The start(inclusive) index of the train data used for 
	 *              creating the tree node content.
	 * @param last The end(exclusive) index of the train data used for creating
	...
	private TreeNodeContent createContent(int first, int last)
	{
		...
		AttributeDelegate classAttributeDelegate = 
				this.attributeDelegates[this.dataSet.getClassAttributeIndex()];
		...
		return new TreeNodeContent(totalWeight, totalClassDistri,
				classification, leafError);
	}

	/**
	 * Select a test attribute from the candidate test attributes.
	 * 
	 * @param first The start(inclusive) index of the train data used for the
	 *              selection of test attribute.
	 * @param last The end(exclusive) index of the train data used for the
	 *             selection of test attribute.
	 * @param testAttrInfo
	 ...
	 */
	private Attribute selectTestAttribute(int first, int last,
			int[] testAttrInfo)
	{
		...
		for (AttributeDelegate attributeDelegate : this.attributeDelegates)
		{
			 ...
			 * Gain value
			 */
			float[] evaluation = attributeDelegate.evaluate(first, last,
			...
			if (gain[gainIndex] > 0
					&& attributeDelegate.getBranchCount() < 0.3 * (this.dataSet
							.getCaseCount() + 1))
			{
		...
		// set very big
		averageGain = ((feasibleTestAttr == 0) ? 100000 : averageGain
				/ feasibleTestAttr);
		...
		for (Attribute attribute : dataSet.getAttributes())
		{
			...
			// attributes
			float gainRatio = GainCalculator.computeGainRatio(gain[gainIndex],
					splitInfo[gainIndex], averageGain);
		...
		if (testAttribute != null)
		{
			...
			if (testAttribute instanceof ContinuousAttribute)
			{
				testAttrInfo[1] = this.attributeDelegates[bestAttrIndex]
						.findCutRank(winSplitIndex, winPreSplitIndex);
			}
