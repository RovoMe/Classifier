...
class TreePruner
{
	 ...
	 */
	TreePruner(DataSet dataSet, AttributeDelegate[] attributeDelegates, 
			TreeNode root, DecisionTree dt)
	{
	 ...
	 *              pruning.
	 * @param last The end(exclusive) index of the train data used for
	 *             pruning.
	 * @param update
	 *            whether the current pruning is a trial to retrieve the
	 *            error after pruning (update = false) or an actual
	 *            pruning (update = true).
	 * @return The estimated error after completing pruning the subtree
	 *         started from the current tree node.
	 */
	private float ebpPrune(TreeNode node, int first, int last,
			boolean update)
	{
		...
		int testAttributeIndex = indexOf(internalNode
				.getTestAttribute().getName(), this.dataSet.getMetaData()
				.getAttributeNames());
		...
		{
			groupBegin = testAttributeDelegate.groupForward(first,
					last, -1, branchDistri);
		}
		...
		for (int index = 0; index < testBranchCount; index++)
		{
			...
			int split = testAttributeDelegate instanceof ContinuousAttributeDelegate 
			...
			{
				nextGroupBegin = testAttributeDelegate.groupForward(
						groupBegin, last, split, branchDistri);
			}
			...
			else if (groupBegin > missingBegin)
			{
				...
				float ratio = branchDistri[index + 1]
						/ (content.getTrainWeight() - branchDistri[0]);
				...
				estimatedTreeError += ebpPrune(internalNode
						.getChildAt(index), missingBegin,
						nextGroupBegin, update);
		...
		float estimatedBranchError = ebpPrune(internalNode
				.getChildAt(maxBranch), first, last, false);
		...
		{
			ebpPrune(internalNode.getChildAt(maxBranch), first, last,
					true);
	...
	/**
	 * Recreate a tree node content with the specified data based on the
	 * tree node's existing content.
	 * 
	 * @param first The start(inclusive) index of the train data used for
			...
			TreeNode node)
	{
		...
		{
			int classLabel = classAttributeDelegate
					.getClassBranch(this.cases[i]);
			totalClassDistri[classLabel] += this.weight[cases[i]];
		}

		// Find the original classification of the tree node
		String nodeClassification = node.getContent()
				.getClassification();
		...
		// prune data
		float basicLeafError = totalWeight
				- totalClassDistri[maxClassIndex];
		float extraLeafError = Estimator.getExtraError(totalWeight,
				basicLeafError);
		float estimatedLeafError = basicLeafError + extraLeafError;

		return new TreeNodeContent(totalWeight, totalClassDistri,
				classification, estimatedLeafError);
	}
