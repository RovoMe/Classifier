...
public class HtmlTreeView extends TreeViewAdapter
{
	...
	/**
	 * Initialize a html view for the tree with the specified tree node as its
	 * root.
	 * 
	 * @param root The root node which acts as a parent for this instance
	 */
	...
	public void initHead()
	{
		...
		buffer.append(
				...
				.append(TreeView.CR).append(TreeView.CR);
		buffer.append("<html>").append(TreeView.CR).append(TreeView.CR);
		buffer.append("<head>").append(TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP).append("<title>").append(title)
				.append("</title>").append(TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP)
				.append("<link rel=\"StyleSheet\" href=\"dTree/dtree.css\" type=\"text/css\" />")
				.append(TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP)
				.append("<script type=\"text/javascript\" src=\"dTree/dtree.js\"></script>")
				.append(TreeView.CR);
	...
	public void initBody()
	{
		...
		buffer.append(TreeView.LEVEL_GAP).append(
				"<p><a href=\"javascript: dt" + count
						+ ".openAll();\">open all</a> ").append(
				"| <a href=\"javascript: dt" + count
						+ ".closeAll();\">close all</a></p>").append(
				TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP).append(
				"<script type=\"text/javascript\">").append(TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP).append(TreeView.LEVEL_GAP).append(
				"dt" + count + " = new dTree('dt" + count + "');").append(
				TreeView.CR);
		...
		{
			buffer.append(TreeView.LEVEL_GAP).append(TreeView.LEVEL_GAP)
					.append("dt" + count + ".add(").append(map.get(node))
					.append(", ");
			if (node.isRoot())
			{
				buffer.append("-1").append(", '").append(
						correct(node.toString())).append("');").append(
						TreeView.CR);
			}
			else
			{
				buffer.append(correct(map.get(node.getParent()).toString()))
						.append(", '").append(correct(node.toString())).append(
								"');").append(TreeView.CR);
			}
		...
		buffer.append(TreeView.LEVEL_GAP).append(TreeView.LEVEL_GAP).append(
				"document.write(dt" + count + ")").append(TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP).append(TreeView.LEVEL_GAP).append(
				"dt" + count + ".openAll()").append(TreeView.CR);
		buffer.append(TreeView.LEVEL_GAP).append("</script>").append(
				TreeView.CR);
	...
	/**
	 * Map the tree nodes in the tree started from the specified tree node to a
	 * hash map preorderly.
	 * 
	 * @param from The source node to add to the map in depth-first order
	 * @param map The map to add the respective nodes with their id (number of
