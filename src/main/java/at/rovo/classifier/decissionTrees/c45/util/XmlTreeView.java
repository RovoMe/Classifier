...
public class XmlTreeView extends TreeViewAdapter
{
	...
	public void initHead()
	{
		...
		buffer.append("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>").append(
				TreeView.CR).append(TreeView.CR);
	 ...
	 * preorderly.
	 */
	private void preorderToBuffer(TreeNode node, StringBuilder buffer,
			String prefix, String gap)
	{
		buffer.append(prefix).append("<").append(getNodeType(node)).append(">")
				.append(TreeView.CR);
		buffer.append(prefix).append(gap).append("<content>").append(
				correct(node.toString())).append("</content>").append(
				TreeView.CR);

		if (node instanceof LeafNode)
		{
			buffer.append(prefix).append("</").append(getNodeType(node))
