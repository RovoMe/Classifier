...
public class DecisionTree implements TreeClassifier
{
     ...
     *         Actually the output of this method, recording the weight distribution of the test data in different class
     *         values.
     * @param weight
    ...
    private void classifyDownward(TreeNode node, String[] record, float[] testClassDistribution, float weight)
    {
        ...
        else
        {
            ...
            /*
             * If the test attribute value of the test data is missing or not exists in declaration, the test data is
             * then passed to all the children tree nodes with the partitioned weight of
            ...
            else
            {
                ...
                for (TreeNode childNode : children) {
                    TreeNodeContent childContent = childNode.getContent();
                    float childWeight = weight * childContent.getTrainWeight() / content.getTrainWeight();
                    classifyDownward(childNode, record, testClassDistribution, childWeight);
                }
            }
        }
    }
    ...
    /**
     * Find the branch index of the child tree node to which the parent tree node should classify the test data to.
     *
     * @param value
     *         The attribute value of the test data on the parent tree node's test attribute
     * @param node
     *         The parent tree node which need to classify the test data to its offspring.
     */
