...
import at.rovo.classifier.decissionTrees.c45.TreeClassifier;
import at.rovo.classifier.decissionTrees.c45.tree.*;
...
public class DecisionTree implements TreeClassifier
{
	...
	public int getTestError(String[][] testData)
	{
		...
		for (int i = 0; i < classificationResults.length; i++)
			if (!classificationResults[i]
					.equals(testData[i][classAttributeIndex]))
				testError++;
		return testError;
	}
	...
	public String[] classify(String[][] testData)
	{
		...
		for (int testIndex = 0; testIndex < testData.length; testIndex++)
		{
			...
			classifyDownward(root, testData[testIndex], testClassDistribution,
					1.0f);
	 ...
	 */
	private void classifyDownward(TreeNode node, String[] record,
			float[] testClassDistribution, float weight)
	{
		...
		if (node instanceof LeafNode)
		{
			...
			if (content.getTrainWeight() <= 0)
			{
				...
				int classificationIndex = indexOf(content.getClassification(),
						dataSet.getClassValues());
			...
			{
				float[] trainClassDistribution = content
						.getTrainClassDistribution();
				for (int i = 0; i < testClassDistribution.length; i++)
				{
					testClassDistribution[i] += weight
							* trainClassDistribution[i]
							/ content.getTrainWeight();
				}
			}
		}
		...
		else
		{
			...
			int testAttributeIndex = indexOf(testAttribute.getName(), dataSet
					.getMetaData().getAttributeNames());
			if (!record[testAttributeIndex].equals("?"))
			{
				int branchIndex = findChildBranch(record[testAttributeIndex],
						(InternalNode) node);
				classifyDownward(node.getChildAt(branchIndex), record,
						testClassDistribution, weight);
			}
			/*
			...
			else
			{
				...
				for (int i = 0; i < children.length; i++)
				{
					...
					float childWeight = (float) weight
							* childContent.getTrainWeight()
							/ content.getTrainWeight();
					classifyDownward(children[i], record,
							testClassDistribution, childWeight);
				}
			}
		}
	}
	...
	private int findChildBranch(String value, InternalNode node)
	{
		...
		if (testAttribute instanceof ContinuousAttribute)
		{
			...
			return (continValue < (node.getCut() + Parameter.PRECISION)) ? 0
					: 1;
		}
		else
		{
			...
			String[] nominalValues = ((DiscreteAttribute) testAttribute)
					.getNominalValues();
	...
	public void prune()
	{
		TreePruner tp = new TreePruner(this.dataSet, this.attributeDelegates, 
				this.root, this);
