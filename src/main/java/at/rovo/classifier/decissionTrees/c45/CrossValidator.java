...
public class CrossValidator
{
    ...
    /**
     * One recorded result of the cross validation. For decision tree classifier, the size of constructed need to be
     * recorded.
     */
    private float[] treeSizes;
    /**
     * The other recorded result of the cross validation: the test error ratios in different folds of cross validation.
     */
     ...
     * <p>
     * It first shuffles, clusters and distributes the provided cross validation data, so that the class-uniformly
     * distributed data can be produced.<br> Then a classifier is built on the train data in each fold, followed with
     * the test data classified using the classifier to get its test error ratio.
     *
     * @param dataSet
     *         The data set to be cross validated
     * @param fold
     ...
     * <p>
     * For example, the sequence of the original class attributes values is {a b a c b}, then after clustering, it
     * becomes {a a b b c}.
     */
    ...
    /**
     * Distributes the cross validation data uniformly in different class values. It is used to produce uniform class
     * distribution for each fold of cross validation.
     * <p>
     * For example, the original sequence of the class value is {a a a b b b c c c c}, after distribution, it becomes {a
     * b c c | a b c | a b c} (suppose 3 fold cross validation).
     */
    ...
    /**
     * Partition the cross validation data to different train data and test data in different folds, build a classifier
     * for each train data and use it to classify the corresponding test data. The cross validation result of each fold
     * is all recorded.
     */
    private void crossValidate()
    {
        ...
        for (int i = 0; i < fold; i++)
        {
            ...
            if (i == fraction)
            {
                testSize = crossValidationData.length / fold;
            }
            ...
            float errorRatio = (100.0f * error / testCount);
    ...
    /**
     * Cross validate a data set with the specified data set name. <br> Usage: java CrossValidator dataSetName [fold].
     */
    public static void main(String[] args)
    {
        CrossValidator cross;

        if (args.length > 2 || args[0].equalsIgnoreCase("-h") || args[0].equalsIgnoreCase("-help"))
        {
        ...
        System.out.printf("%.1f(%.2f)       %.1f%%(%.2f)\n\n", Statistics.mean(treeSizes), Statistics.std(treeSizes),
                          Statistics.mean(testErrorRatios), Statistics.std(testErrorRatios));
    }
}
