 ...
 * It not only contains a basic tree node content, it also contains some
...
public class InternalNode extends TreeNode
{
	...
	public InternalNode(TreeNodeContent content, Attribute testAttribute)
	{
		...
		if (testAttribute instanceof ContinuousAttribute)
			...
			this.children = new TreeNode[((DiscreteAttribute) testAttribute)
					.getNominalValuesCount()];
	...
	/**
	 * Get the cut value of the internal node, if its test attribute is
	 * continuous
	 * 
	...
	/**
	 * Set the cut value of the internal node, if its test attribute is
	 * continuous
	 * 
	 *@param cut The value to cut the node
	 */
	...
	/**
	 * Get the rank of the cut value of the internal node, if its test attribute
	 * is continuous
	 * 
	...
	/**
	 * Set the rank of the cut value of the internal node, if its test attribute
	 * is continuous
	 * 
	 * @param cutRank The rank to cut the value of the internal node
	 */
	...
	public String toString()
	{
		...
		if (!isRoot())
		{
			Attribute parentTestAttribute = ((InternalNode) parent)
					.getTestAttribute();
			...
			{
				sb.append(childIndex == 0 ? " <= " : " > ").append(
						((InternalNode) parent).getCut());
			}
			else
			{
				sb.append(" = ").append(
						((DiscreteAttribute) parentTestAttribute)
								.getNominalValues()[childIndex]);
			}
		}
		else
			sb.append(name);
