...
{
    /**
     * The confidence level table and the corresponding standard standardDeviations table which can be used to
     * interpolate the user specified confidence value and its corresponding standard deviation.
     */
    ...
    /**
     * Estimate the extra error ratio for the specified basic error ratio of the data. <br> Transplanted from Quinlan's
     * c4.5(Release 8).
     *
     * @param totalWeight
    ...
    public static float getExtraError(float totalWeight, float leafError)
    {
        ...
        {
            float Pr = (leafError + 0.5f + SQUAREDSTD / 2 + (float) Math.sqrt(
                            SQUAREDSTD * ((leafError + 0.5f) * (1 - (leafError + 0.5f) / totalWeight) + SQUAREDSTD / 4))
