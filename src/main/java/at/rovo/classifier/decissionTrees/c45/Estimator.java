...
{
	/**
	 * The confidence level table and the corresponding standard
	 * standardDeviations table which can be used to interpolate the user
	 * specified confidence value and its corresponding standard deviation.
	 */
	private static final float[] confidenceLevels = { 0f, 0.001f, 0.005f,
			0.01f, 0.05f, 0.10f, 0.2f, 0.4f, 1.00f };
	private static final float[] standardDeviations = { 4.0f, 3.09f, 2.58f,
			2.33f, 1.65f, 1.28f, 0.84f, 0.25f, 0.00f };
	...
	public static void setConfidence(float confidence)
	{
		...
		if (confidence < 0 || confidence > 1)
		{
			throw new IllegalArgumentException(
					"Confidence level must belong to (0,1)");
		}
		...
		// Interpolate the standard standardDeviations of the specified
		// CONFIDENCE
		float std = standardDeviations[i - 1]
				+ (standardDeviations[i] - standardDeviations[i - 1])
	 ...
	 * @param leafError
	 *            the leaf error of the tree node when considered as a leaf
	 *            node.
	...
	public static float getExtraError(float totalWeight, float leafError)
	{
		...
		{
			return totalWeight
					* (1 - (float) Math.exp(Math.log(CONFIDENCE) / totalWeight));
		}
		else if (leafError < 1 - Parameter.PRECISION)
		{
			float val = totalWeight
					* (1 - (float) Math.exp(Math.log(CONFIDENCE) / totalWeight));
		...
		{
			float Pr = (leafError + 0.5f + SQUAREDSTD / 2 + (float) Math
					.sqrt(SQUAREDSTD
							* ((leafError + 0.5f)
									* (1 - (leafError + 0.5f) / totalWeight) + SQUAREDSTD / 4)))
