...
/**
 * The model created by the SVM algorithm
 *
...
public class Model implements Serializable
{
    ...
    /**
     * Loads a model from the file and returns it as an object.
     *
     * @param modelFileName
     *         The name of the file o load the data from
     *
    ...
    /**
     * Saves a trained model and stores the data into a file <em>modelFileName</em> is pointing to.
     * <p>
     * The data will be stored in the following format:
     * <p>
     * <code> svm_type string<br/> kernel_type string<br/> gamma double<br/> nr_class int<br/> total_sv int<br/> rho
     * double<br/> label int<br/> nr_sv int<br/> SV<br/> double int:double int:double ...<br/> double int:double
     * int:double ...<br/> </code>
     * <p>
     * Note that <em>label</em> and <em>nr_sv</em> are stored for every class. So if there are two classes there will be
     * two entries separated by a comma for <em>label</em> and <em>nr_sv</em>.
     * <p>
     * The entries for SV always are lead by the class label of the sample and followed by the enumeration of the
     * features, where the part before the semicolon (:) is the index part and the part after the semicolon the value
     * part for each feature. The features should be sorted in an ascending order by the index part.
     *
     * @param modelFileName
     *         The name of the file so save the model to
     *
     * @throws IOException
     *         If the provided model file could not get accessed
     */
    public void save(String modelFileName) throws IOException
    {
        ...
        if (KernelType.POLYNOMIAL.equals(param.kernelType) || KernelType.RBF.equals(param.kernelType) ||
            KernelType.SIGMOID.equals(param.kernelType))
        {
            fp.writeBytes("gamma " + param.gamma + "\n");
        }

        if (KernelType.POLYNOMIAL.equals(param.kernelType) || KernelType.SIGMOID.equals(param.kernelType))
        {
    ...
    /**
     * Returns the SVM type of the model.
     *
    ...
    /**
     * Returns the number of classes the model contains.
     *
    ...
    /**
     * Fills a provided array of labels with the labels contained in this model.
     *
     * @param label
     *         The array that should be filled with labels contained in the model
     */
    public void getLabels(int[] label)
    {
        if (this.label != null)
        {
            System.arraycopy(this.label, 0, label, 0, nrClass);
        }
    }

    /**
     * Fills a provided array of indices with the indices contained in this model.
     *
     * @param indices
     *         The array that should be filled with indices contained in the model
     */
    public void getSVIndices(int[] indices)
    {
        if (this.svIndices != null)
        {
            System.arraycopy(svIndices, 0, indices, 0, numInstances);
        }
    }
    ...
    /**
     * Returns the probability for a regression based model.
     *
     * @return The probability if the model is a regression model, 0 instead
     */
    public double getSVRProbability()
    {
        if ((SVMType.EPSILON_SVR.equals(this.param.svmType) || SVMType.NU_SVR.equals(this.param.svmType)) &&
            this.probA != null)
        {
            return this.probA[0];
        }
    ...
    /**
     * Predicts the class a certain sample belongs to.
     *
     * @param x
     *         A sample consisting of multiple features
     *
    ...
    public double predict(Node[] x)
    {
        int nr_class = this.nrClass;
        double[] decValues;
        if (SVMType.ONE_CLASS.equals(this.param.svmType) || SVMType.EPSILON_SVR.equals(this.param.svmType) ||
    ...
    /**
     * Predicts the values for each feature inside the node by solving a quadratic programming problem with linear
     * constraints by utilizing a kernel function which transforms the problem into a further space to simplify the
     * problem.
     * <p>
     * This approach can be thought of as instead of drawing a non-linear separation line (winding line) between the
     * classes, the kernel function modifies the the space in a form that the separation line is a linear line and the
     * classes adept to the new space instead.
     *
     * @param x
     *         The sample containing the features to predict values for
     * @param decValues
     *         An array to be filled with values by the method to enable decision support for choosing the best support
     *         vectors
     *
     * @return The predicted class the sample belongs to
     *
     * @link http://www.dtreg.com/svm.htm
     */
    ...
    public double predictProbability(Node[] x, double[] prob_estimates)
    {
            ...
            this.probA != null && this.probB != null)
        {
            ...
            for (i = 0; i < nr_class; i++)
            {
                ...
                {
                    pairwise_prob[i][j] =
                            Math.min(Math.max(sigmoidPredict(dec_values[k], this.probA[k], this.probB[k]), min_prob),
                                     1 - min_prob);
    ...
    {
        if ((this.probA != null && this.probB != null &&
             (SVMType.C_SVC.equals(this.param.svmType) || SVMType.NU_SVC.equals(this.param.svmType))) ||
            (this.probA != null &&
             (SVMType.EPSILON_SVR.equals(this.param.svmType) || SVMType.NU_SVR.equals(this.param.svmType))))
        {
            return 1;
        }
        else
