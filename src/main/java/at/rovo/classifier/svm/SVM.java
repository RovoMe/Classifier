...
public class SVM extends Classifier<Node[], Double>
{
    ...
    private void sigmoidTrain(int l, double[] dec_values, List<Double> labels, double[] probAB)
    {
        ...
        for (iter = 0; iter < max_iter; iter++)
        {
            ...
            if (stepsize < min_step)
            {
                ...
                {
                    LOG.debug("Line search fails in two-class probability estimates\n");
                }
                break;
            }
        }
        ...
        if (iter >= max_iter)
        {
            if (LOG.isDebugEnabled())
            {
                LOG.debug("Reaching maximal iterations in two-class probability estimates\n");
            }
        }
        probAB[0] = A;
        probAB[1] = B;
    ...
    private double svrProbability(Problem prob, Parameter param)
    {
        ...
        if (LOG.isDebugEnabled())
        {
            LOG.debug("Prob. model for test data: target value = predicted value " +
    ...
    public Double classify(Node[][] items)
    {
        ...
        for (Node[] nodes : items)
        {
            classification.add(this.classify(nodes));
        }
