...
/**
 *  <p>A Support Vector Machine (SVM) performs classification by constructing an 
 *  N-dimensional hyperplane that optimally separates the data into two categories. 
 *  SVM models are closely related to neural networks. In fact, a SVM model using 
 *  a sigmoid kernel function is equivalent to a two-layer, perceptron neural 
 *  network.</p>
 *  <p>Support Vector Machine (SVM) models are a close cousin to classical 
 *  multilayer perceptron neural networks. Using a kernel function, SVM’s are an 
 *  alternative training method for polynomial, radial basis function and 
 ...
 *  in standard neural network training.</p>
 *  <p>In the parlance of SVM literature, a predictor variable is called an attribute, 
 *  and a transformed attribute that is used to define the hyperplane is called 
 *  a feature. The task of choosing the most suitable representation is known as 
 *  feature selection. A set of features that describes one case (i.e., a row of 
 *  predictor values) is called a vector. So the goal of SVM modeling is to find 
 *  the optimal hyperplane that separates clusters of vector in such a way that 
 *  cases with one category of the target variable are on one side of the plane 
 *  and cases with the other category are on the other size of the plane. The 
 *  vectors near the hyperplane are the support vectors.</p>
 *  <p>This implementation is taken from libSVM and got modified to fit into
 *  the available classification framework. The original implementation was done
 *  by Chih-Chung Chang, Chih-Jen Lin.</p>
 *  
 *  
 * @author Chih-Chung Chang, Chih-Jen Lin, Roman Vottner
 * @see www.csie.ntu.edu.tw/~cjlin/libsvm/
 */
public class SVM extends Classifier<Node[],Double>
{
	...
	/**
	 * <p>Initializes a new support vector machine.</p>
	 * 
	 * @param param The parameters necessary to initialize the appropriate type
	 *              of SVM used to train and classify data
	 */
	...
	/**
	 * <p>Platt's binary SVM Probablistic Output: an improvement from Lin et al.
	 * </p>
	 * 
	 * @param l
	 * @param dec_values
	...
	/**
	 * <p>Cross-validation decision values for probability estimates</p>
	 * 
	 * @param prob
	 * @param param
	 * @param Cp
	...
	private void binarySVCProbability(Problem prob, Parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for (i = 0; i < nr_fold; i++)
		{
			...
			subprob.x = new ArrayList<Node[]>(subprob.numInstances);
			subprob.y = new ArrayList<Double>(subprob.numInstances);
	...
	private void groupClasses(Problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
	...
	/**
	 * <p>Trains a model based on the available data in the problem according
	 * to the parameters provided.</p>
	 * <p>Training a support vector machine consists of finding the optimal 
	 * hyperplane, that is, the one with the maximum distance from the nearest
	 * training patterns. The support vectors are these nearest to the 
	 * hyperplane.</p>
	 * 
	 * @param prob The training data
	 * @param param The parameters provided to distinguish what kind of model
	 *              should be build
	...
	private Model train(Problem prob, Parameter param)
	{
		...
		if (SVMType.ONE_CLASS.equals(param.svmType)
				|| SVMType.EPSILON_SVR.equals(param.svmType)
				|| SVMType.NU_SVR.equals(param.svmType))
		{
			...
			if (param.probability == 1	&& (SVMType.EPSILON_SVR.equals(param.svmType) || SVMType.NU_SVR.equals(param.svmType)))
			{
		...
		else
		{
			...
			for (i = 0; i < nr_class; i++)
				for (int j = i + 1; j < nr_class; j++)
				{
					...
					sub_prob.x = new ArrayList<Node[]>(sub_prob.numInstances);
					sub_prob.y = new ArrayList<Double>(sub_prob.numInstances);
					int k;
					for (k = 0; k < ci; k++)
					{
	...
	/**
	 * <p>Invokes the appropriate kernel function for the training set according
	 * to the provided parameter.</p>
	 * 
	 * @param prob The training data
	 * @param param The parameters passed to the application
	 * @param Cp
	 * @param Cn
	...
	/**
	 * <p>Performs a cross-validation for the current trained model.</p>
	 * 
	 * @param param The parameters specifying how the cross-validation should
	 *              be performed
	 */
	public void crossValidation(Parameter param)
	{
		...
		if (SVMType.EPSILON_SVR.equals(param.svmType)
				|| SVMType.NU_SVR.equals(param.svmType))
		{
			for (i = 0; i < prob.numInstances; i++)
			{
	...
	/**
	 * <p>Performs the actual cross validation.</p>
	 * 
	 * @param param The parameters specifying how the cross-validation should
	 *              be performed
	 * @param nr_fold 
	...
	private void crossValidation(Parameter param, int nr_fold, double[] target)
	{
		...
		if ((SVMType.C_SVC.equals(param.svmType) || SVMType.NU_SVC.equals(param.svmType))
				&& nr_fold < l)
		{
			int[] tmp_nr_class = new int[1];
			int[][] tmp_label = new int[1][];
			...
			groupClasses(prob, tmp_nr_class, tmp_label, tmp_start,
					tmp_count, perm);
		...
		for (i = 0; i < nr_fold; i++)
		{
			...
			if (param.probability == 1	&& (SVMType.C_SVC.equals(param.svmType) 
					|| SVMType.NU_SVC.equals(param.svmType)))
			{
				double[] prob_estimates = new double[submodel.getNrClass()];
				for (j = begin; j < end; j++)
					target[perm[j]] = submodel.predictProbability(
							prob.x.get(perm[j]), prob_estimates);
			}
			else
				for (j = begin; j < end; j++)
	...
	/**
	 * <p>Checks the provided parameters for their feasibility and returns the
	 * appropriate error message in case a certain parameter or the problem
	 * statement is not feasible.</p>
	 * 
	 * @param prob  The problem to be checked for feasibility
	 * @param param The parameters passed to the application
	 * @return
	 */
	String checkParameter()
	{
		...
		if (!SVMType.C_SVC.equals(svmType) && !SVMType.NU_SVC.equals(svmType)
				&& !SVMType.ONE_CLASS.equals(svmType)
				&& !SVMType.EPSILON_SVR.equals(svmType)
				&& !SVMType.NU_SVR.equals(svmType))
			return "unknown svm type";

		// kernel_type, degree
		KernelType kernelType = param.kernelType;
		if (KernelType.LINEAR.equals(kernelType)
				&& KernelType.POLYNOMIAL.equals(kernelType)
				&& KernelType.RBF.equals(kernelType)
				&& KernelType.SIGMOID.equals(kernelType)
		...
		if (SVMType.C_SVC.equals(svmType)
				|| SVMType.EPSILON_SVR.equals(svmType)
				|| SVMType.NU_SVR.equals(svmType))
			if (this.param.C <= 0)
				return "C <= 0";
	...
	{
		List<Double> classification = new ArrayList<Double>();
		for (Node[] node : items)
	...
	/**
	 * <p>Returns the trained model if it already exists. If it was not created
	 * before, the method tries to create it.</p>
	 * <p>On creating the model, first the feasibility of the parameters are 
	 * checked and afterwards a default value for the gamma-parameter is used
	 * if none was specified and in case a precomputed kernel should be used the
	 * validity of the training data is checked too.</p>
	 * 
	 * @return
	 */
	...
	/**
	 * <p>Persists a trained model in the file specified in the <em>
	 * modelFileName</em> field of the {@link Parameter} instance.</p>
	 * <p>If the model was not trained before, the method will automatically
	 * train the model before persisting it.</p>
	 * 
	 * @throws IOException If the model could not be persisted
	 */
