...
class SVMPredict
{
	private static PrintInterface printNull = new PrintInterface()
	{
	...
	private static PrintInterface printStdout = new PrintInterface()
	{
		public void print(String s)
		{
			System.out.print(s);
	...
	private static PrintInterface printString = printStdout;
	...
	private static void predict(BufferedReader input, DataOutputStream output, Model model, int predict_probability) throws IOException
	{
		...
		while (true)
		{
			...
			double v;
			if (predict_probability == 1
					&& (SVMType.C_SVC.equals(svmType) || SVMType.NU_SVC.equals(svmType)))
			{
	...
	public static void main(String argv[]) throws IOException
	{
		...
		try
		{
			...
			if (predict_probability == 1)
			{
				if (model.checkProbabilityModel() == 0)
				{
